...
#              Montreal, Toronto, and Vancouver NHPI series.
# =============================================================================
...
aseason_fc <- function(train, holdout, alpha, beta, gamma, level, slope, season, iprint = FALSE) {
  ...
  for (i in 2:n_holdout) {
    ...
    season[k_prev] <- s_new
    fc <- level_new + slope_new + season[spos(i)]
    fcvec[i] <- fc
    sse <- sse + (holdout[i] - fc)^2
    level_prev <- level_new
    slope_prev <- slope_new
...
# Note: switched to ets() for additive seasonal because HoltWinters failed to converge
